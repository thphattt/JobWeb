...
> **Domain đã chốt:** `truyenthongtanchauthanh.vn` — mua tại Vietnix, đứng tên khách (theo **`DOMAIN.md`**).
> **Nhà cung cấp đã chốt:** Vietnix — domain + VPS + DNS cùng 1 tài khoản portal.vietnix.vn.

**Kiến trúc:**

```
Người dùng → DNS (Vietnix) → Nginx trên VPS Vietnix (SSL Let's Encrypt, port 80/443)
...
**→ Làm theo quy trình chi tiết trong `DOMAIN.md`**: đăng ký `truyenthongtanchauthanh.vn` tại Vietnix, tài khoản + chủ thể đứng tên khách (tổ chức), kèm **hồ sơ bản khai chủ thể `.vn`** bắt buộc sau khi mua. Không cần thongbaotenmien.vn (thủ tục đó chỉ dành cho tên miền quốc tế). Chi phí: ~670k năm đầu, gia hạn ~460k/năm.

### 1b. VPS

**Đã chốt: Vietnix** (https://vietnix.vn/vps/) — datacenter tại VN, đáp ứng yêu cầu "máy chủ ở Việt Nam", cùng tài khoản portal.vietnix.vn với domain (Bước 2 của `DOMAIN.md`).

**Gói mua: VPS Cheap 1** — 2 vCPU / 2GB RAM / 40GB SSD, ~159k/tháng, backup tuần.

| Lưu ý gói | Chi tiết |
|---|---|
| Băng thông quốc tế | Dòng Cheap chỉ ~10Mbps quốc tế — đủ vì web phục vụ người dùng VN. Nếu khách cần truy cập nhiều từ nước ngoài → chọn dòng VPS thường/NVMe |
| RAM 2GB | Bắt buộc thêm swap 2GB (Bước 2.6) — build Next.js nặng |
| Nâng cấp | Cần êm hơn → gói 4GB RAM (build nhanh, khỏi lo swap); nâng sau được, không cần mua ngay |

Khi đặt mua: chọn hệ điều hành **Ubuntu 24.04 LTS**, thanh toán theo năm nếu có chiết khấu.

Sau khi mua, Vietnix gửi email: **IP máy chủ + tài khoản root**. Ghi lại cẩn thận.
...
3. Tạo file `.env` trên server:
   ...
   NEXT_PUBLIC_SITE_URL=https://truyenthongtanchauthanh.vn
...
server {
    listen 80;
    server_name truyenthongtanchauthanh.vn www.truyenthongtanchauthanh.vn;
...
Vào portal.vietnix.vn → Quản lý tên miền → DNS (chi tiết trong `DOMAIN.md` Bước 5), thêm bản ghi:

| Loại | Tên | Giá trị |
|---|---|---|
| A | `@` | IP VPS |
| A (hoặc CNAME) | `www` | IP VPS (hoặc `truyenthongtanchauthanh.vn`) |

Chờ phân giải (vài phút–vài giờ), kiểm tra tại https://dnschecker.org. Thử `http://truyenthongtanchauthanh.vn` — thấy site (chưa có khóa) là DNS đã đúng.
...
sudo certbot --nginx -d truyenthongtanchauthanh.vn -d www.truyenthongtanchauthanh.vn
```
Chọn redirect HTTP→HTTPS khi được hỏi. Certbot tự gia hạn (kiểm tra: `sudo certbot renew --dry-run`). Xong: `https://truyenthongtanchauthanh.vn` phải hiện khóa.

## Bước 9 — Thiết lập sau deploy

1. `https://truyenthongtanchauthanh.vn/admin` → tạo tài khoản admin đầu tiên (mật khẩu mạnh).
2. Nạp dữ liệu mẫu nếu cần (chạy từ máy local, PowerShell):
   ```powershell
   $env:SEED_BASE="https://truyenthongtanchauthanh.vn/api"; $env:SEED_EMAIL="<email admin>"; $env:SEED_PASSWORD="<mật khẩu>"; node scripts/seed.mjs
   ```
   (các script khác: `seed-hero-slides.mjs`, `seed-news.mjs`, `seed-projects.mjs`, `seed-p2.mjs`, `attach-images.mjs`)
3. **Email form liên hệ (Resend):** dashboard Resend → **Domains** → add `truyenthongtanchauthanh.vn` → thêm bản ghi SPF/DKIM vào DNS Vietnix (portal.vietnix.vn) → chờ verify → sửa `CONTACT_FROM_EMAIL="Tân Châu Thành <lienhe@truyenthongtanchauthanh.vn>"` trong `.env` → `pm2 restart jobweb`.
...
- [ ] Domain đăng ký xong theo `DOMAIN.md` (đứng tên khách, bản khai chủ thể `.vn` đã duyệt)
- [ ] VPS Vietnix active tại datacenter VN (chụp màn hình thông tin gói làm bằng chứng cho khách)
...
- [ ] Bàn giao cho khách: tài khoản portal.vietnix.vn (domain + VPS), IP + tài khoản VPS, tài khoản /admin, lịch gia hạn domain + VPS, file backup mẫu

## Chi phí ước tính năm đầu

| Khoản | Chi phí |
|---|---|
| Domain `.vn` tại Vietnix (theo `DOMAIN.md`) | ~670k năm đầu, gia hạn ~460k/năm |
| VPS Vietnix Cheap 1 (2 vCPU/2GB/40GB) | ~1,9 triệu/năm (159k/tháng) |
| Nâng gói 4GB (tùy chọn) | ~3–4 triệu/năm |
| Resend | Free (3.000 email/tháng) |
| SSL Let's Encrypt | Free |
| **Tổng (Cheap 1 + .vn)** | **~2,4–2,5 triệu đ/năm đầu** (VPS ~1,78tr đã gồm VAT + domain ~670k) |

Giá thay đổi thường xuyên — chốt theo https://vietnix.vn/vps/ và https://vietnix.vn/bang-gia-ten-mien/ tại thời điểm mua.
